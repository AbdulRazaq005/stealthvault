...
import {
  reEncryptVaultKeyWithNewPassword,
...
const Profile: React.FC = () => {
  ...
  // Current Passwords
  const [currentLoginPassword1, setCurrentLoginPassword1] = useState("");
  const [currentLoginPassword2, setCurrentLoginPassword2] = useState("");
  ...
  const handleUpdateLoginPassword = async (e: React.FormEvent) => {
    ...
    try {
      await apiService.updatePassword({
        password: currentLoginPassword1,
        newPassword: newLoginPassword,
        vaultKeyEncMaster: null,
      });
      toast({
        title: "Success",
        description: "Login password updated. Please log with new password.",
      });
      ...
      setCurrentLoginPassword1("");
      setCurrentLoginPassword2("");
      logout();
    } catch (err: unknown) {
      const message =
  ...
  const handleUpdateMasterPassword = async (e: React.FormEvent) => {
    ...
    if (newMasterPassword === currentMasterPassword) {
      toast({
        title: "Error",
        description:
          "New Master password must different from current Master password.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      const currentVaultkey = await verifyMasterPassword(
      ...
      if (!currentVaultkey) {
        toast({
          title: "Error",
          description: "Current Master password is incorrect.",
          variant: "destructive",
        });
        return;
      }
      const newVaultKeyEncMaster = await reEncryptVaultKeyWithNewPassword(
        currentVaultkey,
        newMasterPassword,
        salt,
      );
      await apiService.updatePassword({
        password: currentLoginPassword2,
        vaultKeyEncMaster: newVaultKeyEncMaster,
        newPassword: null,
      });
      toast({
        title: "Success",
        description:
          "Master password updated. Please unlock your vault with the new master password.",
      });
      setNewMasterPassword("");
      setConfirmMasterPassword("");
      setCurrentMasterPassword("");
      ...
      const message =
        err instanceof Error ? err.message : "Failed to update master password";
      toast({ title: "Error", description: message, variant: "destructive" });
    } finally {
      ...
      >
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input
                    ...
                    value={currentLoginPassword1}
                    onChange={(e) => setCurrentLoginPassword1(e.target.value)}
        ...
        <Card className="vault-card mb-6">
          <CardHeader>
            ...
            <CardDescription>
              Your master password encrypts your vault key. Keep Changing it
              with time to improve security.
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input
                    ...
                    value={currentLoginPassword2}
                    onChange={(e) => setCurrentLoginPassword2(e.target.value)}
                    placeholder="Current login password"
                  />
                  <Button
                    type="button"
